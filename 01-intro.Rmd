...
```{r initializefigs2,echo=FALSE,message=FALSE,warning=FALSE}
...
options(scipen = 99, digits = 3, warnings=F)
#devtools::install_github("crsh/citr")#see https://github.com/crsh/citr

#nb to preview this chapter, preview_chapter("01-intro.Rmd")
# for whole book: render_book("index.Rmd")
```
...
Showing that an intervention works in an individual case is very hard -- especially when dealing with a condition that fluctuates or is self-limiting\index{self-limiting conditions}. In later chapters we shall consider how we can make use of group studies to evaluate interventions, and how single case designs can sometimes give greater confidence that a real effect has been achieved. But first, it is important to recognize the range of factors that conspire to make it difficult to answer the question "Did I make a difference?" To do this, we need to understand about two factors that can mask or mimic real intervention effects: random error and systematic bias. **Random error**\index{random error}\index{bias!random} is used to describe influences that do not alter the average outcome, but which just make it liable to vary capriciously from one occasion to the next. **Systematic bias**\index{bias!systematic} refers to influences that lead outcomes to move in one direction, which may mimic an intervention effect.

```{r weightplot-1,echo=FALSE, include=TRUE,message=FALSE, warnings=FALSE, fig.cap="Random fluctuations in Bridget Jones' weight, measured to the nearest pound",fig.alt="Line chart showing random fluctuations in weight over 14 days",out.width="75%"}
makefig<-0
if(makefig==1){
...
p <- ggplot(data=df, aes(x=Day, y=round(Weight.lbs,0), group=1)) +
  geom_line(color=colorchoice)+  #slategray for bw version and save in images_bw
...
p3<-p2 +labs(y='Weight to nearest lb')+
 ...
 p3
 ggsave(paste0(myfolder,'/weightplot.png'),p3,scale = 1, width = 6, height = 3, units = c("in"), dpi = 300)
}
if (makefig==0){
  knitr::include_graphics("images_bw/weightplot.png")
}
```
...
\begin{center}
```{r weightman,out.width="25%",warnings=F,echo=F,include=TRUE,message=FALSE}
knitr::include_graphics("images_bw/scales_man.jpeg")
```
\end{center}
...
```{r weightplot2,echo=FALSE, message=FALSE, warnings=FALSE, fig.cap="Baseline and outcome measures for an ineffective diet",fig.alt = "14 lines showing pre- post intervention weight, with those showing weight increase in red, and those showing a decrease in black", out.width="75%"}
if(makefig==1){
...
if(bwversion==2){
  colorchoice<-'red'
  myfolder <-'images_colour'}
df$wt2.lb <- .5*df$Weight.lbs+.5*rnorm(14,130,1) #outcome wt as function of baseline + noise
df2<-df[,1:3]
...
mycolour <- c('black',colorchoice)
p1 <- ggplot(data=df3, aes(x=Time, y=Wt, group=Child,col=mycolour[LoseWt])) +
  geom_line() + geom_point() +theme_bw() + scale_x_continuous(breaks=c(1,2)) + theme(legend.position = "none") + scale_color_manual(values=c("black", colorchoice))
...
#   dpi = 300)
p3 <- p1 +labs(y='Weight in lb')+
    theme(axis.title = element_text(size = 18),axis.text=element_text(size=14)) 
p3
 ggsave(paste0(myfolder,'/weightplot2.png'),p3,scale = 1, width = 6, height = 3, units = c("in"), dpi = 300)
}
if(makefig==0){
  knitr::include_graphics("images_bw/weightplot2.png")
}
```
