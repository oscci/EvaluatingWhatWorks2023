...
```{r darts, echo=F,  fig.width=8,fig.height=4,fig.cap='Reliability and validity illustrated as a sharpshooter hitting a target',out.width="75%"}
...
knitr::include_graphics(paste0(myfolder,'/correlationfig.png'))
```
...
makereliabage <- 0 #set to 1 to run this chunk and recreate figure
if(makereliabage==1){
...
mycolours<-c('white','black','lightgray')
alldata$mycol <- as.factor(c(rep(mycolours[1],myn),rep(mycolours[2],myn),rep(mycolours[3],myn)))
#exclude middle group for this plot - too hard to show 3 ages in b&w clearly!
alldata <- alldata[alldata$age!=4,]
allcor <- round(cor(alldata$X1,alldata$X2),2)
alldata$age<-factor(alldata$age)

allcor3 <- round(cor(alldata$X1[alldata$age==3],alldata$X2[alldata$age==3]),2)
allcor5 <- round(cor(alldata$X1[alldata$age==5],alldata$X2[alldata$age==5]),2)
myp <- ggplot(alldata,aes(x=X1,y=X2))+
  geom_point(aes(color=age))+
   theme_bw() +
   xlab("Time 1 Score") +
   ylab("Time 2 Score") +
   ggtitle(paste0("Overall r = ",allcor)) + 
   scale_colour_manual(name="Age (yr)",values=c("black","gray"))

 ggsave(paste0(myfolder,'/reliability-by-age.png'),myp,scale = 1, width = 6, height = 4, units = c("in"), dpi = 300)
...
#### Item Response Theory {.unnumbered}
...
### Generalizability of results to other outcomes: the concepts of far and near transfer
...
```{r makezDensity,echo=FALSE,fig.cap="Distribution of z-scores, showing proportions of people obtaining scores below -1, between -1 and 1, and above 1.",fig.width=6,fig.height=3,out.width="75%"}
...
```{r zdensityCDF,echo=F,include=T,fig.width=6,fig.height=3,fig.cap="Distribution of z-scores, showing proportions of people obtaining scores below -1, between -1 and 1, and above 1"}

knitr::include_graphics(paste0(myfolder,'/ZDensityCDFpng'))
```
...
![](http://127.0.0.1:8999/chunk_output/s/7B7285A4/c5ttuynsm5y3c/000010.png?resize=0)
...
```{r MLU-plot,echo=FALSE,warning=TRUE,message=TRUE,fig.cap='Mean length of utterance (MLU) values for a cohort of 630 children speaking North American English in the Child Language Data Exchange System archive. Recreated from @ratner2016 (data kindly provided by authors)',out.width="75%"}
 ...
 knitr::include_graphics(paste0(myfolder,'/MLU_plot.png'))
