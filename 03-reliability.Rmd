...
<!---#### Types of numerical measurement {.unnumbered}
<p id="custom">-->
...
```{r reliability-by-age,echo=F,include=T,fig.cap="Age range can affect reliability",out.width="80%"}
...
<!---#### How scaled scores are derived {.unnumbered}

::: {#custom}-->

\begin{tcolorbox}[enhanced,breakable,colback=Black!5!lightgray,colframe=black!75!black,coltitle=white,title=How scaled scores are derived]\label{box:scaledscores}
A scaled score gives an indication of how a given raw score compares to a standardization sample. For instance, if there are 80 6-year-olds in the standardization sample, and their mean raw score is 44, with a standard deviation of 5, then we can see that a raw score of 39 would be one standard deviation below the mean, and a raw score of 34 would be two standard deviations below the mean.  
Now suppose we also have normative data on 9-year-olds,\index{data!normative} and their mean raw score is 46, with standard deviation of 4. In this case, a raw score of 39 is 1.75 standard deviation below the mean, and a raw score of 34 is three standard deviations below the mean. So the same raw score represents a more serious deficit in a 9-year-old than in a 6-year-old, because the typical (normative) score of 9-year-olds is higher than that of 6-year-olds.  
...
\begin{center}  
```{r ZDensityCDF,echo=F,include=T,fig.cap="Distribution of z-scores, showing proportions of people obtaining scores below -1, between -1 and 1, and above 1",out.width="65%"}

knitr::include_graphics(paste0(myfolder,'/ZDensityCDF.png'))
```
\end{center}  
\index{scaled scores}\index{z-scores}\index{percentile scores}\index{centile scores}
You may wonder why bother with scaled scores, given that percentiles have a more intuitive interpretation. Percentiles are good for communicating how exceptional a given raw score is, but they are not so good for doing statistical analyses. For those interested in understanding why, [this blogpost: http://deevybee.blogspot.com/2011/04/short-nerdy-post-about-use-of.html](http://deevybee.blogspot.com/2011/04/short-nerdy-post-about-use-of.html) gives a short explanation.
...
```{r makeMLUplot,echo=FALSE,warning=TRUE,message=TRUE,fig.cap='Mean length of utterance (MLU) values for a cohort of 630 children speaking North American English in the Child Language Data Exchange System archive. Recreated from @ratner2016 (data kindly provided by authors)',out.width="75%"}
