...
```{r maketrogchangeplot,echo=FALSE,message=FALSE,warning=FALSE}
...
makefig <- 0  #assume figure already created but retain code to create it
if(makefig==1){
...
p3 <- p + 
  ...
  theme(axis.title.x = element_text(size=14), 
        axis.text.x = element_text(size=12), legend.position = 'top',legend.text = element_text(size=10))
if(bwversion==1){
  p3 <- p3+scale_fill_grey()
  } 

 
ggsave(paste0(myfolder,'/trogchangefig.png'),p3,scale = 1, width = 5, height = 3.5, units = c("in"), dpi = 300)
}
```

```{r trogfig, echo=F,message=F,warning=F,fig.width=5,fig.height=3.5,fig.cap="TROG outcome data from a study by Bishop et al 2006"}
...
makeregfig <- 1 #don't run this section unless you want to recreate figure
if(makeregfig==1){
...
p1<- p+  geom_vline(xintercept =(-1),linetype="dashed", color = "darkgrey", linewidth=1)+ylab('Test t2')+xlab('Test t1')+ 
...
if(bwversion==1){
  p1<-p1+scale_colour_manual(values=c("darkgray","black"))
  } 


ggsave(paste0(myfolder,'/reg2mean.png'),p1,scale = 1, width = 6, height = 3.5, units = c("in"), dpi = 300)
}
