...
An alternative to stratification, which has become popular in recent years, is adaptive randomization [@hoare2013]. For each new individual, the allocation to treatment ratio changes based on:
...
knitr::kable(mini.mat,escape = F, align = "c", booktabs = T,caption = 'Example: randomized allocation by minimization') %>%
  kable_styling(c("striped", "bordered"), latex_options = "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  group_rows("Sex",1,2) %>%
