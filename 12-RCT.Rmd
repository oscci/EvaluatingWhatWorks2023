...
-   Understand the difference between a one-tailed and two-tailed test, and know when each is appropriate
...
dsdat$preIntervention <- dsdat$letter_sound_t1
dsdat$outcome <- dsdat$letter_sound_t2
pirateplot(outcome~group,theme=1,cex.lab=1.5,data=dsdat, point.o=1, xlab="", ylab="Post-intervention score")
...
ggscatter(
  dsdat, x = "preIntervention", y = "outcome",
...
res.aov <- dsdat %>% anova_test(outcome ~ preIntervention +group)
# mylm1 <- lm(posttest~group,data=dsdat) #equivalent to aov.1
mylm <- lm(outcome~preIntervention+group,data=dsdat) #equivalent to res.aov
...
source <-c('Intercept','PreIntervention','Group')
...
vars1<-c('subject_id','group','preIntervention')
...
long_all$time[1:nrow(dsdat)]<-'Baseline (preIntervention)'
long_all$dummy<-1
...
1.  Find an intervention study of interest and check whether the protocol was deposited in a repository before the study began. If so, check the analysis against the preregistration, cf. @goldacre2019. Note which analysis method was used to estimate the intervention effect.\
