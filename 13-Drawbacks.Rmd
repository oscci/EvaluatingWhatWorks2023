...
```{r steps-porz,include=T,echo=FALSE,message=FALSE,warning=FALSE,out.width="75%"}
options(kableExtra.html.bsTable = T)
# dt13 <- matrix(c(
...
# colnames(dt13) <- c('Step',	'Explanatory (ideal conditions)',	'Pragmatic (real-world health care)') %>% stringr::str_trim()
# # 
# dt13 %>% write_excel_csv("Data/table-13-2023-03-19.csv")

dt13 <- tibble('Step' = c(1:3),	
               'Explanatory (ideal conditions)' = letters[1:3],
               'Pragmatic (real-world health care)' = LETTERS[1:3])

knitr::kable(dt13,escape = F, align = "c", booktabs = T,
             caption="Steps to assess efficacy (explanatory trial) and effectiveness (pragmatic trial)") %>%
  kable_styling(c("striped", "bordered"), latex_options = "striped", full_width = F) 
