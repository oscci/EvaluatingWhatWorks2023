...
```{r interventionhet, echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Data on body weight in chicks given a new diet (Intervention) or regular diet (Control). Average linear regression lines of intervention and controls group line is shown as solid black and dotted lines respectively.",fig.alt="Figure where x-axis shows time (days) and y-axis shows body weight, with lines showing increase in weight over time for individual chicks. Although the bold lines depicting averages show an effect of diet, there is considerable variability from chick to chick.", out.width="90%"}
...
myg <- ggplot(data=ChickWeight2,aes(x = Time, y = Outcome, group = Chick)) + geom_line(aes(colour=Intervention),alpha=0.5) +
    ...
    xlab("Time (days)") + ylab("Weight (gm)")+
  geom_line(data = predicted_df1, aes(x=Time, y=Outcome),linetype='dashed') + 
  geom_line(data = predicted_df2, aes(x=Time, y=Outcome))
if(bwversion==1){
  myg<-myg+  scale_colour_grey() + theme_bw()
}
ggsave(here(myfolder,'interventionhet.png'),myg,scale = 1, width = 6, height = 4, units = c("in"), dpi = 300)
}
