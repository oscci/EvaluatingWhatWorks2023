...
```{r crossoverplot, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Idealized results from a crossover RCT",out.width="75%"}
...
mod2 <- lm(y~x,newdat2)
 ...
 ggplot(dat[!dat$intervention==2,], aes(x = x, y = y)) + 
   ...
   annotate("text",x=1.5,y=7.5,label="Group A have intervention",size=4.5) +
   annotate("text",x=3,y=7.5,label="Group B have intervention",size=4.5)+
...
geom_segment(aes(x=2,y=6,xend = 2.5, yend = 4),col="red",linetype="dashed",size=2) +
   ...
   annotate("text",x=1.5,y=6.2,label="Group A",color="red",size=5) +
   annotate("text",x=1.5,y=4.2,label="Group B",color="blue",size=5)+theme(axis.text = element_blank(),axis.ticks = element_blank())
...
```
```{r waitlist-plot, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Idealized results from a wait list design RCT",out.width="75%"}
...
```{r calderfig, echo=F,fig.cap = "Mean % correct in delayed cross-over study by Calder et al, 2021 (data plotted from Calder et al's Table 2).",out.width="75%"}
