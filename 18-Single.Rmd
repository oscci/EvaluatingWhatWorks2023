...
```{r Nitems-power,include=T,echo=F,warning=F,message=F,out.width="75%"}
options(kableExtra.html.bsTable = T)
...
if(readthisfile==1){
  mytab <- readr::read_csv('data/power_singlecase_itemssets.csv')
}
mytab<- mytab %>% select(1:7)
colnames(mytab)<-c("N per Set",".1",".2",".3",".4",".5",".6")
...
```{r ebbelsfig-old,echo=F,include=F,warning=F,message=F,fig.cap = "Individual results for targets and controls at each term (Redrawn Fig 3 from Leniston & Ebbels, data kindly provided by Susan Ebbels)",out.width="75%"}
#knitr::include_graphics("images/Ebbels.png")
#fig was created with makeebbels
```


```{r ebbelsfig,echo=F,include=T,warning=F,message=F,fig.cap = "Test caption",out.width="75%"}
knitr::include_graphics("images/Ebbels.png")
#fig was created with makeebbels
```
