...
```{r crossoverplot, echo=F, message=F, warning=F, fig.cap="Schematic of a cross-over RCT",fig.alt="Line chart showing outcome scores for two groups across Phase 1, Washout (Phase 2)  and Phase 3. Group A is treated during Phase 1, and group B is treated during Phase 3. Group A scores higher than Group B during Phase 1, but scores then decline during Washout, and Group B scores higher than Group A during Phase 3.", out.width="85%"}
...
```{r waitlist-plot,  echo=F, message=F, warning=F, fig.cap="Schematic of a waitlist RCT",fig.alt="Line chart showing outcomes for two groups who differ in terms of the point at which intervention is administered. Group A scores higher than Group B during Period 1, and gains persist, with Group B showing benefits in Period and so catching up with Group A.",out.width="80%"}
...
png("images_colour/calderfig-1.png", width = 500, height = 350,unit='px') #change folder for bw
  
 #Try with base R!
plot(1, type = "n",                         # Remove all elements of plot
     xlab = "Phase", ylab = "% Correct",
     xlim = c(1, 7), ylim = c(0, 80))
mycols <- c('blue','red')  #for bw version use black and grey
mylines <- c(1,2,3)
mypch<-c(2,0,19)
vpos<-c(75,10)
hpos <-c(4.5,6.4)
