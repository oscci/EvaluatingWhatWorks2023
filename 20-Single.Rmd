...
```{r SingleCaseDplot, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Outcome over time in a single case ABA design. Redrawn from digitized data from two participants from Figure 2 of Armson & Stuart (1998)",out.width="85%"}
makefig<-0
if(makefig==1){
sub1<-read.csv('data/stutterP5_read.csv')
sub2<-read.csv('data/stutterP6_read.csv')
png("images_bw/SingleCaseDplot.png", width = 550, height = 300,unit='px')
par(mfrow=c(1,2))
df<-sub1
for (i in 1:2){
  if(i==2){df<-sub2}
plot(df$x,df$y,type='b',pch=15,main=paste0('Participant ',(i+4)),ylim=c(0,25),ylab='% Stuttering',xlab='Time (mins)')
abline(v=5,lty=2)
abline(v=15,lty=2)
...
```{r bestplot, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Outcome over time in multiple outcomes design. Reconstructed data from 16 participants, Best et al (2013)",out.width="75%"}
makefig <- 0
...
if(makefig==1){
png("images_bw/bestplot.png", width = 450, height = 300,unit='px')
...
lines(bestdf$Cpost,bestdf$Tpost,type='p',col='darkgrey',pch=16)
for(i in 1:nrow(bestdf)){
  segments(bestdf$Cpre[i],bestdf$Tpre[i],bestdf$Cpost[i],bestdf$Tpost[i])
}
abline(a=0,b=1,lty=2)
legend(1, 95, legend=c("Pre-intervention", "Post-intervention"),pch=16,col=c('black','darkgrey') ,cex=0.8)
dev.off()
}
...
```{r swainfig,echo=F,include=T,warning=F,message=F,fig.cap = "Data from one case from  Swain et al (2020). The shaded region shows sessions with intervention for each of the three outcomes.",out.width="60%"}
...
```{r ebbelsfig,echo=F,include=T,warning=F,message=F,fig.cap = "Individual results for targets and controls at each term",out.width="90%"}
...
```{r SingleCaseDplot2, echo=F, message=F, warning=F, fig.width=8,fig.height=4, fig.cap="Outcome over time in a single case ABA design. Digitized data from two participants from Figure 2 of Armson & Stuart (1998)",out.width="85%"}
makeplot <-0
if(makeplot==1){
sub1<-read.csv('data/stuttering_read_P11.csv')
sub2<-read.csv('data/stuttering_read_P12.csv')
png("images_bw/SingleCaseDplot2.png", width = 550, height = 300,unit='px')
par(mfrow=c(1,2))
df<-sub1
for (i in 1:2){
  if(i==2){df<-sub2}
plot(df$x,df$y,type='b',pch=15,col='black',main=paste0('Participant ',(i+10)),ylim=c(0,25),ylab='% Stuttering',xlab='Time (mins)')
abline(v=5,lty=2)
abline(v=15,lty=2)
text(2,24.5,"A",cex=1.5)
text(10,24.5,"B",cex=1.5)
...
```{r kdataplot, echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Group means from Koutsoftas et al, 2009. Filled points show intervention phase, unfilled show baseline or post-intervention",out.width="85%"}
makefig <- 0
...
if(makefig==1){
png("images_bw/kdataplot.png", width = 550, height = 350,unit='px')
...
lines(G2,type='b',xlab='Session',ylab='Mean number correct /3',ylim=c(0,3),col='black',pch=G2p)

text(2.4,2.6,'Baseline',cex=.7,col="black")
text(6.8,2.5,'Intervention',cex=.7,col="black")
text(11.5,2.6,'Post',cex=.7,col="black")
arrows(0.5, 2.5, 4, 2.5,col='black',code=3,length=.1)
arrows(5, 2.4, 10, 2.4,col='black',code=3,length=.1)
arrows(11, 2.5, 12, 2.5,col='black',code=3,length=.1)
...
```{r Calderplus, echo=F,fig.cap = "Mean per cent correct for all 3 probes in delayed cross-over study by Calder et al, 2021 (data from Table 2).",out.width="85%"}
...
png("images_bw/calderfig-2.png", width = 500, height = 350,unit='px')
...
Once you have studied \ref{fig:Calderplus}, consider whether you think the inclusion of the probes has strengthened your confidence in the conclusion that the intervention is effective.
